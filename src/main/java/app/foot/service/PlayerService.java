...
public class PlayerService {
    ...
    public List<Player> updatePlayers(List<Player> toUpdate) {
        for (int i = 0; i < toUpdate.size(); i++) {
            ...
            int IdOfI = i;
            PlayerEntity playerActual = repository.findById(toUpdate.get(i).getId()).orElseThrow(
                    ()->{throw new BadRequestException("can't update new id "+ toUpdate.get(IdOfI).getId());}
            );
            if ( !toUpdate.get(i).getTeamName().equals(mapper.toDomain(playerActual).getTeamName())) {
